...
class FreeImageProvider extends Provider {
        ...
        async upload(buffer) {
                const { data: html } = await axios
                        ...
                        .catch(() => null);
                // FIX: Guard against null html response when GET fails
                if (!html || typeof html !== 'string') {
                        throw new Error("Failed to fetch freeimage.host page");
                }
                const tokenMatch = html.match(/PF\.obj\.config\.auth_token = "(.+?)";/);
                if (!tokenMatch || !tokenMatch[1]) {
                        throw new Error("Failed to extract auth_token from freeimage.host");
                }
...
class CatboxProvider extends Provider {
        ...
        async upload(buffer) {
                ...
                // FIX: Use env var instead of hardcoded credential
...
class Pixhost extends Provider {
        ...
        async upload(buffer) {
                ...
                }).catch(() => { throw new Error("Failed to fetch Pixhost image page"); });
...
class Uploader {
        ...
        async upload(buffer, provider) {
                ...
                try {
                        ...
                        // FIX: Re-throw original error instead of wrapping in new Error (loses stack trace)
                        throw error;
                }
        }
}
