...
export default {
        ...
        command: ["pixelate", "pixel"],
        permissions: "all",
        hidden: false,
        failed: "Failed to %command: %error",
        wait: "⏳ Creating pixelated image...",
        category: "fun",
        cooldown: 10,
        limit: true,
        ...
        async execute(m, { sock }) {
                ...
                try {
                        ...
                        const apiUrl = `https://some-random-api.com/canvas/misc/pixelate?avatar=${encodeURIComponent(avatar)}`;
                        ...
                        await m.reply({
                                image: buffer,
                                caption: `🌀 *Pixelated image for @${who.split("@")[0]}*`,
                        });
                } catch (e) {
                        console.error("[pixelate ERROR]:", e);
                        m.reply(`❌ Failed to pixelate: ${e.message}`);
                }
        },
};
