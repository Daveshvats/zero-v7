...
export default {
        ...
        async execute(m, { sock }) {
                ...
                try {
                        ...
                        const apiUrl = `https://some-random-api.com/canvas/overlay/triggered?avatar=${encodeURIComponent(avatar)}`;
                        ...
                        await m.reply({
                                video: buffer,
                                gifPlayback: true,
                                caption: `😤 *@${who.split("@")[0]} is TRIGGERED!*`,
                        });
                } catch (e) {
                        console.error("[triggered ERROR]:", e);
                        m.reply(`❌ Failed to create triggered gif: ${e.message}`);
                }
        },
};
