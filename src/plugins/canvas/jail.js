...
export default {
        ...
        async execute(m, { sock }) {
                ...
                try {
                        ...
                        const apiUrl = `https://some-random-api.com/canvas/overlay/jail?avatar=${encodeURIComponent(avatar)}`;
                        ...
                        await m.reply({
                                image: buffer,
                                caption: `🔒 *@${who.split("@")[0]} has been sent to jail!*`,
                        });
                } catch (e) {
                        console.error("[jail ERROR]:", e);
                        m.reply(`❌ Failed to create jail image: ${e.message}`);
                }
        },
};
